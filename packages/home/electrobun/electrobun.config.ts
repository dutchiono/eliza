...
export default {
  app: {
    name: "Eliza Home",
    identifier: "ai.eliza.home",
    version: "2.0.0-alpha.22",
    description: "Your AI home companion",
    urlSchemes: ["elizahome"],
  },
  runtime: {
    exitOnLastWindowClosed: false,
  },
  scripts: {
    // Sign native code inside home-dist/node_modules on the inner app bundle
  ...
  build: {
    ...
    watch: ["../dist", "../../autonomous/dist", "src/shared/"],
    ...
    // Eliza Home intentionally supports both desktop WebGPU paths:
    ...
    // Copy the Vite-built renderer (packages/home/dist/) into the bundle as renderer/.
    ...
    copy: {
      ...
      // ElizaOS backend server bundle (compiled package output from packages/autonomous/dist/).
      // agent.ts walks up from import.meta.dir looking for home-dist/ to spawn
      // the canonical runtime entry (`bin.js start`).
      // Paths are relative to packages/home/electrobun/ (where electrobun build is run).
    ...
    mac: {
      ...
      notarize:
        ...
        (process.env.ELIZA_HOME_ELECTROBUN_NOTARIZE ??
  ...
  release: {
    baseUrl:
      ...
      "https://github.com/elizaOS/eliza/releases/download/eliza-home-canary/",
    generatePatch: true,
  },
