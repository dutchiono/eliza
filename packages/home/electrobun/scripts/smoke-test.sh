...
#
# Usage:
#   bash packages/home/electrobun/scripts/smoke-test.sh
#
...
MOUNT_POINT=""
LAUNCH_APP_BUNDLE=""
STARTUP_LOG="$HOME/.config/Eliza Home/eliza-home-startup.log"
...
ensure_diagnostics_dir() {
  if [[ -z "$SMOKE_DIAGNOSTICS_DIR" ]]; then
    SMOKE_DIAGNOSTICS_DIR="$(mktemp -d /tmp/eliza-home-smoke-diagnostics.XXXXXX)"
  fi
...
copy_supporting_diagnostics() {
  ...
  if [[ -f "$STARTUP_LOG" ]]; then
    cp "$STARTUP_LOG" "$SMOKE_DIAGNOSTICS_DIR/eliza-home-startup.log" 2>/dev/null || true
  fi
...
kill_stale_processes() {
  ...
  while IFS= read -r pid; do
    ...
    if kill -0 "$pid" >/dev/null 2>&1; then
      if [[ $found -eq 0 ]]; then
        echo "Stopping stale Eliza Home launcher/backend processes..."
        found=1
      fi
      kill "$pid" >/dev/null 2>&1 || true
    fi
  done < <(
    pgrep -f '/(Applications|tmp|private/tmp|Volumes)/.*Eliza Home[^/]*\.app/Contents/MacOS/launcher|home-dist/(bin|entry)\.js' || true
  )
...
find_live_packaged_pid() {
  ...
  pgrep -f "${bundle_regex}/Contents/MacOS/launcher|${bundle_regex}/Contents/MacOS/bun|${bundle_regex}/Contents/Resources/main\\.js|${bundle_regex}/Contents/Resources/app/bun/index\\.js|${bundle_regex}/Contents/Resources/app/home-dist/(bin|entry)\\.js" | head -1 || true
}
...
verify_packaged_renderer_assets() {
  ...
  if [[ -d "$renderer_dir" ]]; then
    ...
    if [[ ! -d "$renderer_dir/assets" ]]; then
      echo "ERROR: Packaged renderer assets directory missing: $renderer_dir/assets"
      dump_failure_diagnostics "packaged renderer assets directory missing"
      exit 1
    fi
    asset_count="$(find "$renderer_dir/assets" -maxdepth 1 -type f \( -name "*.js" -o -name "*.css" \) | wc -l | tr -d ' ')"
    if [[ -z "$asset_count" || "$asset_count" -lt 1 ]]; then
      echo "ERROR: Packaged renderer assets directory is missing compiled JS/CSS files."
      dump_failure_diagnostics "packaged renderer assets missing compiled bundle files"
      exit 1
    fi
  ...
  if [[ -n "${RUNTIME_ARCHIVE:-}" && -f "$RUNTIME_ARCHIVE" ]]; then
    ...
    asset_count="$(
      tar --zstd -tf "$RUNTIME_ARCHIVE" \
        | grep -E "^${archive_bundle_root}/assets/.+\.(js|css)$" \
        | wc -l \
        | tr -d ' '
    )"
    if [[ -z "$asset_count" || "$asset_count" -lt 1 ]]; then
      echo "ERROR: Packaged renderer assets directory in wrapper archive is missing compiled JS/CSS files."
      dump_failure_diagnostics "packaged renderer assets missing compiled bundle files in wrapper archive"
      exit 1
    fi
...
echo "============================================================"
echo " Eliza Home Electrobun Smoke Test"
echo " Build env  : $BUILD_ENV"
...
if [[ "$SKIP_BUILD" == "1" ]]; then
  ...
  echo "[1/4] Building staged Home desktop bundle (env=$BUILD_ENV)..."
...
DIRECT_RUNTIME_DIR="$APP_BUNDLE/Contents/Resources/app/home-dist"
...
if [[ -n "$MOUNT_POINT" ]]; then
  LAUNCH_APP_DIR="$(mktemp -d /tmp/eliza-home-smoke-app.XXXXXX)"
...
LAUNCHER_STDOUT="$(mktemp /tmp/eliza-home-smoke-launcher.stdout.XXXXXX)"
LAUNCHER_STDERR="$(mktemp /tmp/eliza-home-smoke-launcher.stderr.XXXXXX)"
build_launcher_command
