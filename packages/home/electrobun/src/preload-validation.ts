...
export function readBuiltPreloadScript(
  ...
  if (!status.preloadExists) {
    throw new Error(
      `[Main] preload.js is missing at ${status.preloadPath}. Run \`bun run build:preload\` before launching Eliza Home.`,
    );
  }

  if (status.stale) {
    throw new Error(
      `[Main] preload.js is stale relative to ${status.sourcePath}. Run \`bun run build:preload\` before launching Eliza Home.`,
    );
  }
  ...
  if (!preload.trim()) {
    throw new Error(
      `[Main] preload.js is empty at ${status.preloadPath}. Run \`bun run build:preload\` before launching Eliza Home.`,
    );
  }
