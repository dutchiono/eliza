...
describe("resolveRuntimeNodeModulesPath", () => {
  it("accepts an explicit runtime node_modules path", () => {
    expect(
      resolveRuntimeNodeModulesPath(
        ["/tmp/Eliza Home.app/Contents/Resources/app/home-dist/node_modules"],
        {},
      ),
    ).toBe("/tmp/Eliza Home.app/Contents/Resources/app/home-dist/node_modules");
  });
  ...
  it("derives the runtime node_modules path from the postBuild bundle in ELECTROBUN_BUILD_DIR", () => {
    ...
    const appBundle = path.join(tempDir, "Eliza Home canary.app");
    fs.mkdirSync(
      path.join(
        appBundle,
        "Contents",
  ...
  it("accepts a home-dist directory and appends node_modules", () => {
    expect(resolveRuntimeNodeModulesPath(["/tmp/home-dist"], {})).toBe(
      "/tmp/home-dist/node_modules",
    );
  });
