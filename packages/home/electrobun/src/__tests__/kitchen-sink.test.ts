/**
 * Kitchen Sink Test — Eliza Home Electrobun Desktop App
 *
...
vi.mock("../native/agent", () => ({
  ...
  resolveConfigDir: vi.fn(() => "/mock/config/Eliza Home"),
  getRuntimeDistFallbackCandidates: vi.fn(() => ["/mock/home-dist"]),
}));
...
describe("DesktopManager — tray", () => {
  ...
  it("createTray() creates a Tray with image and title options", async () => {
    await manager.createTray({ icon: "/icon.png", tooltip: "Eliza Home" });
...
describe("CanvasManager — window creation via BrowserWindow", () => {
  ...
  it("createWindow() uses defaults when options are minimal", async () => {
    ...
    const callArg = MockBrowserWindow.mock.calls[0]?.[0];
    expect(callArg.title).toBe("Eliza Home Canvas");
...
describe("Push event routing — sendToWebview dispatches to RPC send proxy", () => {
  ...
  it("PUSH_CHANNEL_TO_RPC_MESSAGE contains expected Eliza Home push events", () => {
    const expected = [
      "agentStatusUpdate",
      "desktopTrayClick",
      "desktopTrayMenuClick",
...
describe("Application menu structure — expected items", () => {
  it("setupApplicationMenu menu definition has Eliza Home, Edit, View, Window menus", () => {
    ...
    expect(menuDef[0].label).toBe("Eliza Home");
...
//
// To verify manually:
//   1. Run `bun run start` (or `bun run dev`) in packages/home/electrobun/
...
describe.skip("INTERACTIVE: Tray icon and menu", () => {
  ...
  it.todo("Tray icon tooltip reads 'Eliza Home' on hover");
...
describe.skip("INTERACTIVE: Deep links and URL schemes", () => {
  it.todo(
    "Opening elizahome:// URL from browser triggers the app's open-url handler",
  );
...
describe.skip("INTERACTIVE: Application menu", () => {
  it.todo(
    "Application menu shows Eliza Home, Edit, View, Window menus in macOS menu bar",
  );
  it.todo("Eliza Home > Check for Updates triggers the updater flow");
  it.todo("Eliza Home > Quit Eliza Home exits the app cleanly");
  it.todo(
