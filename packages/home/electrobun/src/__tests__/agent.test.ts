...
// Set MILADY_DIST_PATH so resolveRuntimeDistPath() never touches import.meta.dir
// (which is Bun-only and undefined in Node/vitest).
const MOCK_DIST_PATH = "/mock/home-dist";
...
describe("AgentManager", () => {
  ...
  beforeEach(async () => {
    ...
    // Default: all filesystem checks return true (dist exists, bin.js exists, etc.)
    const existsSync = await getExistsSyncMock();
    existsSync.mockReturnValue(true);
    manager = new AgentManager();
  });

  describe("home-dist fallback candidates", () => {
    it("prefers the Resources/app runtime path for installed apps", () => {
      const candidates = getRuntimeDistFallbackCandidates(
        "/Applications/Eliza Home-canary.app/Contents/Resources",
        "/Applications/Eliza Home-canary.app/Contents/MacOS/launcher",
      );

      expect(candidates[0]).toBe(
        "/Applications/Eliza Home-canary.app/Contents/Resources/app/home-dist",
      );
      expect(candidates).toContain(
        "/Applications/Eliza Home-canary.app/Contents/home-dist",
      );
    });

    it("includes the sibling home-dist path for extracted app runtimes", () => {
      const candidates = getRuntimeDistFallbackCandidates(
        "/private/tmp/Eliza Home-canary.app/Contents/Resources/app/bun",
        "/private/tmp/Eliza Home-canary.app/Contents/MacOS/launcher",
      );

      expect(candidates).toContain(
        "/private/tmp/Eliza Home-canary.app/Contents/Resources/app/home-dist",
      );
    ...
    it("includes the extracted app runtime path used by self-extracting installs", () => {
      const candidates = getRuntimeDistFallbackCandidates(
        "/tmp/ai.eliza.home/canary/self-extraction/Eliza Home-canary/bin",
        "/tmp/ai.eliza.home/canary/self-extraction/Eliza Home-canary/bin/launcher",
      );

      expect(candidates[0]).toBe(
        "/tmp/ai.eliza.home/canary/self-extraction/Eliza Home-canary/Resources/app/home-dist",
      );
      expect(new Set(candidates).size).toBe(candidates.length);
    });
    ...
    it("includes Windows resources/app runtime candidates beside launcher.exe", () => {
      const candidates = getRuntimeDistFallbackCandidates(
        "/Users/test/AppData/Local/ai.eliza.home/canary/self-extraction/Eliza Home-canary/bin",
        "/Users/test/AppData/Local/ai.eliza.home/canary/self-extraction/Eliza Home-canary/bin/launcher.exe",
      );

      expect(candidates).toContain(
        "/Users/test/AppData/Local/ai.eliza.home/canary/self-extraction/Eliza Home-canary/bin/resources/app/home-dist",
      );
      expect(candidates).toContain(
  ...
  describe("start()", () => {
    ...
    it("spawns bun process with the canonical runtime entry when present", async () => {
      ...
      expect(status.agentName).toBe("Eliza Home");
      ...
      expect(spawnArgs[0][2]).toBe("/mock/home-dist/bin.js");
    ...
    it("uses the bundled Bun executable for installed app launches", async () => {
      Object.defineProperty(process, "execPath", {
        configurable: true,
        value: "/Applications/Eliza Home-canary.app/Contents/MacOS/launcher",
      });
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        [
          "/Applications/Eliza Home-canary.app/Contents/MacOS/bun",
          "run",
          "/mock/home-dist/bin.js",
          "start",
        ],
    ...
    it("defaults agent name to Eliza Home when agents endpoint fails", async () => {
      const mockProc = createMockProcess();
      mockSpawn.mockReturnValue(mockProc);
  ...
  describe("setSendToWebview()", () => {
    it("emits status updates via the callback", async () => {
      ...
      // Make runtime entry resolution fail for the quick error path
      const existsSync = await getExistsSyncMock();
      existsSync.mockImplementation((p: string) => {
        if (
          typeof p === "string" &&
          (p.endsWith("bin.js") || p.endsWith("entry.js"))
        ) {
          return false;
        }
        if (p === MOCK_DIST_PATH) return true;
